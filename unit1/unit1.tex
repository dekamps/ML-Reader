\part{Unit 1 - Recap of Statistics}
...
\section{Notebooks and Worksheets}
The following notebooks contain exercises for this Unit:
\begin{enumerate}
  \item \texttt{01\_Simulating\_Stochastic\_Processes.ipynb}
  \item \texttt{02\_Unfair\_coins\_Maximum\_Likelihood\_and\_Bayes.ipynb}
\end{enumerate}
...
outcomes is called a \emph{sample}, the set size is called  \emph{sample size}.

A \emph{probability distribution} is a set of probabilities defined over a discrete state
...
distribution, so it plays a central role in simulation random processes. In the jupyter notebook \emph{Simulating Stochastic Processes}, you will experiment with how you can simulate arbitrary distributions
...
if conditional probabilities do not express a causal relationship \cite{pearl2016,pearl2018}. Now please consider \emph{Worksheet Bayes} to see a real world example of the
...
$$
p(\mu \mid \mathcal{D} ) = \frac{p(\mathcal{D} \mid \mu)} {p(\mathcal{D})} p(\mu),
$$
where
$$
...
Note the following:
\begin{enumerate}
\item The likelihood and the prior have a very similar mathematical form. This is deliberate: the computation of the posterior is very simple this way. 
...
The likelihood and prior are called  \emph{conjugate} if the posterior and the prior are from the same family of fuctions. Conjugacy determines whether you can do
Bayesian analysis analytically, or whether you have to resort to numerical approximations. It just happens that beta functions are a nice way of picking priors
\emph{and} also are the mathematical form for are likelihood. Cojugacy makes Bayesian analysis analytically tractable. Some important models like linear regression
allow for the choice of conjugate priors, but others like logistic regression do not. In such cases Bayesian analysis must resort to numerical approximations.

The issue of normalisation has been treated casually here. The hard work has been done by introducing the Gamma function, but you are recommended to try  Exercise
